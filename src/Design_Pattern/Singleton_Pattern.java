...
class Singleton{
    private static class singleInstanceHolder{
        private static final Singleton INSTANCE = new Singleton(); // 하나의 객체 인스턴스를 생성하고 새로운 객체 생성
    }
...
public class Singleton_Pattern {
    public static void main(String[] args) {
        Singleton a = Singleton.getInstance(); // a, b라는 모듈은 하나의 인스턴스를 공유하여 사용
        Singleton b = Singleton.getInstance();
        System.out.println(a.hashCode());
        System.out.println(b.hashCode());
        if (a == b) {
            System.out.println(true); // a, b는 같은 인스턴스를 사용하므로 hashcode가 같아서 true 출력
        }
    }
}
